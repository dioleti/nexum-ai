...
class TestSMBFileLoader(unittest.TestCase):
    def test_init_with_client(self):
        client = MagicMock(spec=SMBFileClient)
        loader = SMBFileLoader("path/to/document", client=client)
        self.assertIs(loader.client, client)
        self.assertEqual(loader.path, "path/to/document")

    def test_init_missing_server_or_share(self):
        with self.assertRaises(NexumValueError):
            SMBFileLoader("document.txt", server=None, share="share")

        with self.assertRaises(NexumValueError):
            SMBFileLoader("document.txt", server="server", share=None)

    @patch("nexum.document.storage.loader.smb.SMBFileClient")
    def test_init_creates_client(self, mock_client):
        loader = SMBFileLoader(
            "document.txt",
            server="srv",
            share="shr",
            username="u",
    ...
    def test_load_success(self):
        ...
        loader = SMBFileLoader("document.txt", client=client)
        data = loader.load()
